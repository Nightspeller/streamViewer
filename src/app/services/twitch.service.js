...
var TwitchService = (function () {
    ...
    ;
    TwitchService.prototype.getUserFollows = function (amount, page, user) {
        var _this = this;
        if (user) {
            return new Promise(function (resolve, reject) {
                _this.http.get("https://api.twitch.tv/kraken/users?login=" + user + "&api_version=5", { headers: _this.headers }).toPromise().then(function (userResponse) {
                    _this.http.get("https://api.twitch.tv/kraken/users/" + userResponse.json().users[0]._id + "/follows/channels", { headers: _this.headers }).toPromise().then(function (response) {
                        _this.prepareUserFollows(amount, response.json()).then(function (pickerDisplayGroup) { return resolve(pickerDisplayGroup); });
                    });
                });
            });
        }
    ...
    TwitchService.prototype.prepareUserFollows = function (amount, userFollows) {
        var _this = this;
        var url = "https://api.twitch.tv/kraken/streams?channel=";
        for (var i = 0; i < userFollows.follows.length; i++) {
            url += userFollows.follows[i].channel._id + ',';
        }
        return new Promise(function (resolve, reject) {
            _this.http.get(url, { headers: _this.headers }).toPromise().then(function (data) {
                var pickerDisplayGroup = TwitchService.transformStreamsToPickerDisplayGroup(data.json());
